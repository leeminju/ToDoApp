...
public class SignupRequestDto {
    ...
    @Pattern(regexp = "^(?=.*[a-z0-9]+)(?=.*[a-z0-9])[a-z0-9]{4,10}$", message = "username은 영어 소문자, 숫자 조합만 가능합니다.")
    @Size(min = 4, max = 10, message = "username은 4자 이상 10자 이하로 입력해주세요")
    private String username;

    @NotBlank(message = "password는 공백일 수 없습니다.")
    @Pattern(regexp = "^(?=.*[a-zA-Z0-9]+)(?=.*[a-zA-Z0-9])[a-zA-Z0-9]{8,15}$", message = "password는 영어 대문자, 소문자, 숫자 조합만 가능합니다.")
